...
class EXP4(ContextualMultiArmedBandit):
    ...
    def __init__(self, n_arms, n_experts, gamma=0.1):
        ...
        self.regretHistory = []
        self.means = [1000 for _ in range(n_arms)]
    ...
    def run(self, context=None, **kwargs):
        ...
        for j in range(self.n_arms):
            ...
            for i in range(len(self.advice)):
    ...
    def update(self, reward):
        ...
        # Regret
        if (self.means[self.lastChoice] == 1000):
            self.regretHistory.append(0)
        else:
            self.regretHistory.append(self.means[self.lastChoice] - reward)

        self.rewardHistory[self.lastChoice].append(reward)
        self.totalRewardHistory.append(reward)
        self.means[self.lastChoice] = np.average(self.rewardHistory[self.lastChoice])
